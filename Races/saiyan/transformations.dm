transformation
	saiyan
		...
		super_saiyan
			...
			mastery_boons(mob/user)
				...
				if(user.Potential>=45&&mastery<100)
					mastery=100
				if(user.Potential>=60&&user.transUnlocked<2)
		...
		super_saiyan_2
			...
			forceadd = 0.35
			mastery_boons(mob/user)
				if(user.Potential>=61&&mastery<25)
					mastery=25
				if(user.Potential>=63&&mastery<50)
					mastery=50
				if(user.Potential>=65&&mastery<75)
					mastery=75
				if(user.Potential>=67&&mastery<100)
					mastery=100
				if(mastery >= 100)
					passives = list("Instinct" = 2, "Flow" = 2, "Flicker" = 1, "Pursuer" = 2, "PureDamage" = 3, "PureReduction" = 3, "SaiyanPower2"=0.5)
				if(user.Potential>=70&&user.transUnlocked<3)
